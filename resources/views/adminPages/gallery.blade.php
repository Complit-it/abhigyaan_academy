                    ...
                    <div class="col-md-12">
                        <div class="card card-primary">
                            ...
                            <form role="form" action="/gallery" method="post" enctype="multipart/form-data">
                                @else
                                    <form role="form" action="/edit-gallery" method="post"
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-4">
                                        <label for="exampleInputEmail1">Image Title <span
                                    ...
                                    <div class="form-group col-md-4">
                                        <label for="exampleInputEmail1">Alternate Title <span
                                                style="color: red;">*</span></label>
                                        <input type="text" min="0" class="form-control" placeholder="Name"
                                            @if ($selectSubCatId != null) value="{{ $selectSubCatId->alttitle }}" @endif
                                            name="alttitle" required>
                                    </div>
                                ...
                                <table id="table21" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        @for ($i = 0; $i < count($allPhotos); $i++)
                                            <tr>
                                                <td>{{ $allPhotos[$i]['id'] }}</td>
                                                <td>{{ $allPhotos[$i]['title'] }}</td>
                                                <td>{{ $allPhotos[$i]['alttitle'] }}</td>
                                                <td> <!-- HTML code to open image in modal -->
                                                <a href="{{ url($allPhotos[$i]['file_url']) }}" target="_blank" >Preview</a>
                                                </td>

                                                <td>
                                                    <input type="hidden" value="{{ url($allPhotos[$i]['file_url']) }}"
                                                        id="myInput{{ $allPhotos[$i]['id'] }}">
                                                    <button class= "btn btn-info" onclick="myFunction('{{ url($allPhotos[$i]['file_url']) }}')">Click to Copy Url</button>
                                                </td>
                                                <td>
                                                <a href="/edit-gallery/{{ $allPhotos[$i]['id'] }}"
                                                        class="btn btn-primary btn-sm">Edit</a>
                                                    <a href="/delete-galery/{{ $allPhotos[$i]['id'] }}"
...
function myFunction(text) {
    ...
    // Set the style to ensure it is out of view
