                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">Add MCQ</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="card card-primary">
                            ...
                            <!-- form start -->
                            @if ($selectSubCatId == null)
                            <form role="form" action="/mcqs" method="post" enctype="multipart/form-data">
                                @else
                                    <form role="form" action="/edit-mcq-batch" method="post"
                                        enctype="multipart/form-data">

                                        <input type="hidden" name="id" value="{{ $selectSubCatId->batch_id }}"></input>
                            @endif
                            @csrf
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-4">
                                        <label for="exampleInputEmail1">MCQ Tile <span
                                                style="color: red;">*</span></label>
                                        <input type="text" min="0" class="form-control" placeholder="MCQ Tile"
                                            @if ($selectSubCatId != null) value="{{ $selectSubCatId->title }}" @endif
                                            name="name" required>
                                    </div>

                                    <div class="form-group col-md-4">
                                        <label for="customFile">Select MCQ CSV FIle <a href="/assets/sample/samplemcq.csv">(Downlaod Sample File)</a></label>

                                        <div class="custom-file">
                                        <input type="file" accept="csv/*" name="csv" class="custom-file-input" id="customFile">
                                ...
                                <table id="table21" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        @for ($i = 0; $i < count($allBatches); $i++)
                                            <tr>
                                                <td>{{ $allBatches[$i]['batch_id'] }}</td>
                                                <td>{{ $allBatches[$i]['title'] }}</td>
                                                <td>{{ $allBatches[$i]['subject_name'] }}</td>
                                                <td>{{ $allBatches[$i]['topic_name'] }}</td>
                                                <td>{{ $allBatches[$i]['sub_topic_name'] }}</td>
                                                <td>{{ $allBatches[$i]['sub_sub_topic_name'] }}</td>
                                                <td> <!-- HTML code to open image in modal -->
                                                    <a href="/upload/mcqcsv/{{$allBatches[$i]['batch_id']}}.csv" target="_blank" >Download</a>
                                                </td>
                                                <td>
                                                <a href="/edit-mcq-batch/{{ $allBatches[$i]['batch_id'] }}"
                                                        class="btn btn-primary btn-sm">Edit</a>
                                                    <a href="/delete-mcq-batch/{{ $allBatches[$i]['batch_id'] }}"
